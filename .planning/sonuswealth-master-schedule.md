...
| AU3 | **D-AUTH-1 step-up auth framework** — re-password / TOTP / biometric challenges | ✅ DONE 2026-05-25 (v0: re-password) | `src/lib/step-up.js` (requireStepUp primitive, 5-min elevation window, concurrent-call coalescing, verifyPassword via supabase.auth.signInWithPassword); `src/state/step-up.jsx` (StepUpProvider + modal with autofocus + Escape-cancel); wired into `DataCapture.commitCapture` + manual entry submit. TOTP-level step-up extension uses AU4 plumbing. |
| AU4 | **MFA + biometric** for end-users (TOTP + WebAuthn) | ✅ DONE 2026-05-25 (TOTP) | `src/lib/auth.js` extended: enrollMFA, verifyMFAEnrolment, listMFAFactors, unenrollMFA, getAuthLevel (AAL1/AAL2). New `src/components/Settings/SecuritySettings.jsx` — QR-code Authenticator enrol flow + status + unenrol with step-up gate. WebAuthn (biometric) deferred to AU4-extension — requires custom credential storage table. |
| AU5 | **Permission matrix (X27)** — by document class × accessor role (owner/spouse/IFA/solicitor) | ✅ DONE 2026-05-25 (JS layer) | `src/lib/permissions.js` — 6 roles × 11 document classes × 5 actions allow-list matrix. Exports `isAllowed(role, action, docClass)`, `assertAllowed()`, `allowedActions()`, `readableClasses()`, `matrixSnapshot()`. RLS-policy translation deferred to Vault §4.3 implementation. |
| AU6 | **Pre-launch CTA-honesty review** — confirm all auth-adjacent CTAs labelled correctly until production | ✅ DONE 2026-05-25 | Welcome.jsx CTA changed from "Join the waitlist" to "Get started" (auth is real now). Account.jsx CTAs updated to "Create account" / "Sign in" (was "Join waitlist"). Settings.jsx Security row promoted from phase2-stub to live with SecuritySettings detail panel. Other "(waitlist)" labels (IFA mode, non-UK jurisdictions) intentionally retained — those flows genuinely aren't built. |
