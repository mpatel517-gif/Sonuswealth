...
function themeKeywordMap(theme) {
  ...
  if (t.includes('salary_sacrifice')) return ['salary sacrifice', 'sacrifice']
  if (t.includes('carry_forward')) return ['carry forward', 'carry-forward', 'unused']
  if (t.includes('cgt_realisation') || t === 'cgt_realisation') return ['cgt', 'annual exempt', 'crystallise', 'realise', 'gains']
  if (t === 'eis_vct' || t.includes('eis') || t.includes('vct')) return ['eis', 'vct', 'high-risk']
  if (t.includes('div') && t.includes('salary')) return ['salary', 'dividend', 'paying yourself']
  ...
  if (t.includes('ci_vs_ip')) return ['critical illness', 'income protection']
  if (t.includes('write_in_trust') || (t.includes('trust') && t.includes('protect'))) return ['in trust', 'writing a life']
  if (t.includes('self_employed_ip') || (t.includes('self') && t.includes('ip'))) return ['self-employed', 'income protection']
  if (t.includes('key_person') || t.includes('keyperson')) return ['key-person', 'key person']
  if (t.includes('wol') || t.includes('whole_of_life') || t.includes('term')) return ['whole-of-life', 'term assurance', 'whole of life']
