...
| TL-S-20 | global | Brand string drift — verify zero `Caelixa` / `Finio` user-visible strings on Timeline (per FD-NAME-1). Engine comments (`finio-` prefixes, `finioScore` field) are deferred to Wave 4 Morph sweep but visible UI must already be clean. | FUNCTIONAL |
