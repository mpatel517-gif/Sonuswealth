...
| **ST-AB-D08** | **FAIL (A5 + A6 + FD-NAME-1)** | BRAND-LEAK | **DEMO-BLOCKING** | "Scoring engine · **FINIO-1.0**". Hardcoded literal at Settings.jsx:600. FD-NAME-1 locked 9 May 2026: every `Sonuswealth`/`Finio` in user-facing strings = FAIL. This is the single highest-confidence brand-leak finding on the screen. **Root cause beyond this row:** engine docstring at `src/engine/fq-calculator.js:688` still reads "FINIO-1.0 scoring engine" — so even after this UI string is fixed, the leak survives in the codebase and will resurface anywhere a future surface introspects the engine. **Verify-task confirmed: FINIO-1.0 is present and user-visible.** | Settings.jsx:600 |
...
All four are user-visible occurrences of `FINIO` / `FQ` / `Financial Quotient`. Each violates FD-NAME-1 (locked 9 May 2026, supersedes Sonuswealth/Finio). Per the inventory's own seed list (S-01, S-02) and per founder rule: every Sonuswealth/Finio in user-facing strings = FAIL.
